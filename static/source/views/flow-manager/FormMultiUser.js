...
(function ($) {
    ...
    $.RoleControls.FormMultiUser = function (element, ptions, sheetInfo) {
        ...
        this.FormMultiUserHandler = '/sys/user/';
        this.CpLock = false;
        this.Options = ptions;
    ...
    $.RoleControls.FormMultiUser.Inherit($.RoleControls.RoleBaseControl, {
        ...
        //读取单个选中的loginName
        GetUnitLoingNames: function () {
            var ValObjs = this.GetValue();
            var UintLoginNames = new Array();
            for (var key in ValObjs) {
                return ValObjs[key].loginName
                // UintIDs.push(key);
            }
            return NONE;
        },
        ...
        AddUserID: function (UserID) {
            ...
            if (ids.length > 0) {
                var param = {
                    ActionName: "SheetUserAction", Command: "GetUserProperty", id: JSON.stringify(ids)
                };
                this.Ajax(
                    that.FormMultiUserHandler,
                    "POST",
        ...
        AddChoice: function (UnitObject) {
            ...
            choice.append(NewUnitObject.userName);
        ...
        LoadUsersByParenID: function (parentID) {
            ...
            if (!that.MultiUserData.DepUserItems[parentID]) {
                ...
                for (var i = 0; i < Length; i++) {
                    ...
                    var icon = UnitItem.Type == 1 ? "fa icon-gongsi" : (UnitItem.Type == 2 ? "fa icon-xiashuguanli" : UnitItem.Icon);
