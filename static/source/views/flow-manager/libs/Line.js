...
import { WorkflowElementType, WorkflowMultiActionType } from './Workflow'
import { WorkflowMode } from './WorkflowDocument'
...
//线条类定义
let Line = function (lineConfig, workflow) {
    this.workflow = workflow
    this.svg = this.workflow.svg
    this.workflowMode = undefined
    // Id
    this.Id = undefined;
...
Line.prototype = {
    ...
    SetBlurStyle: function () {
        this.workflow.FocusLine = undefined;
        this.workflow.SelectLine = undefined;
    ...
    draw: function (drawCrossPoints) {
        ...
        //使用DIV绘线
            ...
            if (this.LineDiv)
                $(this.LineDiv).remove();
            var thisLineDiv = this.LineDiv = $("<div></div>").addClass("LineDiv").appendTo(this.workflow.workspace);
            ...
            this.LineDiv.appendTo(this.workflow.workspace);
            ...
            this.Arrow.appendTo(this.workflow.workspace);
        }
        this.SetLineStyle();
        this.SetText();
    },
    ...
    SetText: function () {
        ...
        if (!this.Label) {
            var that = this;
            this.Label = $("<div class='" + LineStyleClassName.Label + "'></div>").attr(LineSettings.Label_LineIDPropertyName, this.Id).appendTo(this.workflow.workspace);
    ...
    Select: function () {
        ...
        //取消选中所有活动
        $(this.workflow.activities).each(function () {
            this.Unselect();
        });

        //取消选中其他线条
        for (var index = 0 ; index < this.workflow.lines.length; index++) {
            if (this.workflow.lines[index].Id != this.Id)
                this.workflow.lines[index].Unselect();
        }
    ...
    selectHandler: function (e) {
        ...
        if ($(e.target).hasClass(LineStyleClassName.LineHandler)) {
            this.workflow.setMultiActionFlag(WorkflowMultiActionType.LineHandle);
                ...
                .bind("mousemove" + LineSettings.LineHandleEventNamespace, function (e) {
                    ...
                    if (LineResizeStack.CurrentHandler) {
                        ...
                        if (LineResizeStack.CurrentHandler.hasClass(LineStyleClassName.StartPointHandler) || LineResizeStack.CurrentHandler.hasClass(LineStyleClassName.EndPointHandler)) {
                            //计算停靠信息
                            var dockPosition = this.workflow.calculateDockActivity(e);
                            ...
                            if (LineResizeStack.CurrentHandler.hasClass(LineStyleClassName.StartPointHandler)) {
                                ...
                                //不停靠
                                    ...
                                    LineResizeStack.CurrentLine.startPoint = {
                                        x: e.pageX - this.svg._offset().left,
                                        y: e.pageY - this.svg._offset().top
                                    };
                        ...
                        //控制对象是线段
                            ...
                            if (prePointIndex) {
                                ...
                                if (!isNaN(x1) && !isNaN(x2) && !isNaN(y1) && !isNaN(y2)) {
                                    //竖直线
                                    if (x1 == x2) {
                                        LineResizeStack.CurrentLine.Points[prePointIndex].x = e.pageX - this.svg._offset().left;
                                        LineResizeStack.CurrentLine.Points[prePointIndex + 1].x = e.pageX - this.svg._offset().left;

                                        //移动当前控制点
                                        $(LineResizeStack.CurrentHandler)
                                            .css("left", x1 / 2 + x2 / 2 - $(this.svg)._position().left);
                                    ...
                                    //水平线
                                        ...
                                        LineResizeStack.CurrentLine.Points[prePointIndex].y = e.pageY - this.svg._offset().top;
                                        LineResizeStack.CurrentLine.Points[prePointIndex + 1].y = e.pageY - this.svg._offset().top;

                                        //移动当前控制点
                                        $(LineResizeStack.CurrentHandler).css("top", y1 / 2 + y2 / 2 + $(this.svg)._position().top);
            ...
            $(document).one("mouseup" + LineSettings.LineHandleEventNamespace, function (e) {
                this.workflow.setMultiActionFlag(WorkflowMultiActionType.None);
                ...
                if (LineResizeStack.IsDrawing) {
                    ...
                    if (!LineResizeStack.CurrentLine.startActivity || !LineResizeStack.CurrentLine.endActivity) {
                        ...
                        this.workflow.removeLine(LineResizeStack.CurrentLine.Id);
    ...
    calculateCrossPoints: function () {
        ...
        $(thisLine.Points).each(function (indexI) {
            if (indexI > 0) {
                ...
                //计算与当前线的交叉点，重绘所有交叉线条
                $(thisLine.workflow.lines).each(function () {
    ...
    showHandler: function () {
        ...
        if (points && points.length > 0) {
            ...
            //显示起点控制点
            handler.clone()
                .css("left", points[0].x + this.svg._position().left)
                .css("top", points[0].y + this.svg._position().top)
                .addClass(LineStyleClassName.StartPointHandler)
                .appendTo(this.svg.parent())
                .bind("mousedown" + LineSettings.LineHandleEventNamespace, this.selectHandler);

            //显示终点控制点
            handler.clone()
                .css("left", points[points.length - 1].x + this.svg._position().left)
                .css("top", points[points.length - 1].y + this.svg._position().top)
        ...
        $(points).each(function (index) {
            if (index > 0 && index < points.length - 2) {
                ...
                if (pre_x == next_x) {
                    ...
                    handler.clone()
                        .data("index", index)
                        .css("left", $(this.svg)._position().left + pre_x)
                        .css("top", $(this.svg)._position().top + pre_y / 2 + next_y / 2)
                        //添加可水平移动的鼠标样式
                ...
                //水平线
                    ...
                    handler.clone()
                        .data("index", index)
                        .css("left", $(this.svg)._position().left + pre_x / 2 + next_x / 2)
                        .css("top", $(this.svg)._position().top + pre_y)
                        //添加可竖直移动的鼠标样式
    ...
    beginDrawing: function () {
        ...
        $(document).unbind(LineSettings.LineHandleEventNamespace).bind("mousemove" + LineSettings.LineHandleEventNamespace, function (e) {
            ...
            if (!LineDrawStack.IsDrawing) {
                this.workflow.setMultiActionFlag(WorkflowMultiActionType.LineDraw);
            ...
            if (LineDrawStack.IsDrawing) {
                ...
                //计算修正停靠点
                var _dockPosition = this.workflow.calculateDockActivity(e);
                if (_dockPosition) {
        ...
        $(document).one("mouseup" + LineSettings.LineHandleEventNamespace, function (e) {
            ...
            if (!LineDrawStack.CurrentLine.endActivity)
                this.workflow.removeLine(LineDrawStack.CurrentLine.Id);
            else {
    ...
    autoDraw: function () {
        ...
        if (LineDrawStack.CurrentLine.endActivity) {
            if (LineDrawStack.CurrentLine.endActivity.ToolTipText == "Start") {
                ...
                for (var index = 0; index < this.workflow.lines.length; index++) {
                    if (this.workflow.lines[index].Id != thisLineId
                        && ((this.workflow.lines[index].startActivity == LineDrawStack.CurrentLine.startActivity && this.workflow.lines[index].endActivity == LineDrawStack.CurrentLine.endActivity)
                            || (this.workflow.lines[index].endActivity == LineDrawStack.CurrentLine.startActivity && this.workflow.lines[index].startActivity == LineDrawStack.CurrentLine.endActivity)))
                        _Drawable = false;
                }
            }
