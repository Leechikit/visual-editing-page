<template>
    <div class="info-modal">
      ...
      <div
        :class="[
          'open-modal-scroll-container', 'info-modal-body',
          {
      ...
      >
        <Form :label-width="80">
            <div>
                ...
                <modifyModal v-if="modModal&&(!(csvData && csvData.length > 0))"  :code="modCode"></modifyModal>
            </div>
        </Form>
        <div id="print-approval-record-title" class="info-title" v-if="csvData && csvData.length > 0">审批记录</div>
        <i-table id="print-approval-record-table" v-show="csvData && csvData.length > 0" :content="self" :loading="getLoading" :columns="columns" :data="csvData" size="small" ref="tableCsv" class="i-table"></i-table>
        <workflow-chart :value="workflowParams"></workflow-chart>
      </div>
      <div class="info-modal-footer">
        <Button type="ghost" v-if="(!modModal)&&!(csvData && (csvData.length>0))&&checkPerm(['act:deal:start:'+getCurrentAppKey])" @click="modShow()">编辑</Button>
         <Button v-if="value.row.Status==2"
          type="primary" :loading="urgLoading"
          @click="urg"
      ...
      <!-- 浏览锚点 -->
        <AnchorPoint v-if="anchorVisible" :anchorList="anchorList" conSelector="#SheetContent"></AnchorPoint>
    </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    delegatePrint() {
      document.querySelector('[data-action=Print]').AppRunId = this.value.row.AppRunId;
    ...
    urg(){
      ...
      HTTP.urge(this.value.row.AppRunId).then((data)=>{
        if(data.code==0){
    ...
    checkAnchorVisible() {
      if ($('#SheetContent') && $('#SheetContent .page-header').length > 0) {
          this.anchorVisible = true
          this.anchorList = Array.prototype.slice.call($('#SheetContent .page-header'))
      }
    }
  }
};
