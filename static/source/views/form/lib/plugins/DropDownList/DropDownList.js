...
(function ($) {
    ...
    DropDownList.prototype = {
        ...
        Render: function () {
            ...
            if (that.Opt.placeholder) {
                that.$DropText.html('<div style=" color: #999;">' + that.Opt.placeholder + '</div>');
            }
        ...
        RenderOptions: function () {
            ...
            for (var i = 0, len = that.OptionAyy.length; i < len; i++) {
                ...
                if((!that.OptionAyy[i])||that.OptionAyy[i]=='undefined'){
                    continue
                }
                var opt = that.OptionAyy[i];
                if(that.OptionAyy[i].indexOf('module*&Name')>=0){
                    that.$DropList.append('<li class="drop-item-item-label" data-index="' + i + '" data-value="' + opt + '"><i class="ivu-icon ivu-icon-md-add"></i>' + that.Options[opt] + '</li>');

                }
                else{ var opt = that.OptionAyy[i];
        ...
        Disabled: function (disabled) {
            this.Opt.disabled = disabled;
            if (disabled) {
                this.$DropDown.children('.drop-handle').css('background-color', '#f2f2f6');
                this.$DropDown.find('.drop-btn').css('background-color', '#f2f2f6');
            } else {
                this.$DropDown.children('.drop-handle').css('background-color', 'white');
                this.$DropDown.find('.drop-btn').css('background-color', 'white');
            }
        }
    }
...
(function ($) {
    ...
    SelectForm.prototype = {
        ...
        SetValue: function (val) {
            ...
            if (val !== "" && val != void 0) {
                ...
                for (var i = 0, len = items.length; i < len; i++) {
                    ...
                    if (obj) {
                        ...
                        if (that.Options.IsMultiple == true) {
                            //多选
                            this.$Items.children("li.dropdownlist-item[data-value='" + obj.Value + "']").find("input[type=checkbox]").prop("checked", true)
                        } else {
                            //单选
                            this.$Items.children("li.dropdownlist-item[data-value]").removeClass("active");
                            this.$Items.children("li.dropdownlist-item[data-value='" + obj.Value + "']").addClass("active")
                        }
                    }
                    else {
                        console.log("设置的值不存在");
            ...
            this.Value = val == void 0 ? "" : val;
            // this.Value = val==void 0 || "";
...
// WEBPACK FOOTER //
// ./src/lib/plugins/DropDownList/DropDownList.js