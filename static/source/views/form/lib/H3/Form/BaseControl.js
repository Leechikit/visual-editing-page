(function ($) {
    ...
    $.fn.JsControl = function () {
        ...
        $(this).each(function () {
            ...
            //数据项
            var datafield = $control.data($.Controlkey.DataFieldKey.toLocaleLowerCase());
            var controlkey = $control.data($.Controlkey.SheetControlKey.toLocaleLowerCase());
            if (!controlkey) {
                return jControl;
            }
            var dataitem = $.myControls.GetSheetDataItem(datafield, $control);

            if (args.length > 0) {
                dataitem = $.extend(dataitem, args[0]);
            }
            jControl = $.Controlkey.Run.call($control, controlkey, [dataitem]);
    ...
    $.myControls = {};
    //控件属性
    $.myControls.GetDefaultOptions = function (controlKey) {
        var p = {};
    ...
    // 读取表单数据
    $.myControls.GetSheetDataItem = function (datafield, $control) {
...
(function ($) {
    ...
    //parm [options] 组件的参数
    $.myControls.BaseControl = function (element, options, ResponseContext) {
        // 表单信息
    ...
    //基础属性
    $.myControls.BaseControl.prototype = {
        ...
        PreRender: function () {
            if (this.IsMobile) {
                ...
                else if (controlkey == "FormAttachment" || controlkey == "FormQuery"||controlkey == "FormFolder"||controlkey == "FormCitySelect") {
            ...
            if (this.IsMobile) {
                ...
                else if (controlkey == "FormAttachment" || controlkey == "FormButton" || controlkey == "FormSns" || controlkey == "FormTaskTips"||controlkey == "FormFolder"||controlkey == "FormCitySelect") {
        ...
        InitComputationRule: function () {
            ...
            if (this.ResponseContext.IsCreateMode ||
                this instanceof $.myControls.FormFormula ||
                ...
                if (field.indexOf(".") > -1) {//规则字段在子表
                    if (this.DataField.indexOf(".") > -1) {//配置规则的字段在子表
                        ...
                        if (childSchemaName1 == childSchemaName2) {//同一个子表
                            ...
                            var gridMgr = $("div.sheet-control[data-datafield='" + field.slice(0, field.indexOf(".")) + "']").JsControl();
                            if (gridMgr) {
                                    ...
                                    gridMgr.BindChange(eventName, function (args) {
                                        ...
                                        window[eventName] = setTimeout(function () {
                                            ...
                                            for (var i = 0; i < targetCtrls.length; i++) {
                                                $(targetCtrls[i]).JsControl().SetComputationResult(computationRule, computationRuleFields);
                                            }
                                            //that.SetComputationResult(computationRule, computationRuleFields);
                                        }, 600);
                                    });
                        ...
                        for (var i = 0; i < tdCtrl.length; i++) {
                            var controlMgr = $(tdCtrl[i]).JsControl();
                            if (controlMgr) {
                                controlMgr.UnbindChange(eventName);
                                controlMgr.BindChange(eventName, function () {
                                    that.SetComputationResult(computationRule, computationRuleFields);
                    ...
                    eventName = "change.cr." + this.DataField;
                    var changeControl = $("div.sheet-control[data-datafield='" + field + "']").JsControl();
                    if (!changeControl) {
                        continue;
                    }
        ...
        GetComputationResult: function (rule, fields) {
            ...
            for (var j = 0, len = fields.length; j < len; j++) {
                ...
                if (ctrlField.indexOf('.') > -1) {
                    ...
                    if (ctrlKey == 'FormNumber' || (ctrlKey == "FormFormula" && ctrl.JsControl().BindType == ctrl.JsControl().BindControlType.Number)) {
                        val.push(ctrl.JsControl().GetNum());
                    } else if (ctrlKey == 'FormTextBox' || ctrlKey == 'FormTextArea') {
                        val.push('"' + ctrl.JsControl().GetValue() + '"');
                        ...
                        val.push("'" + ctrl.JsControl().GetValue() + "'");
                    } else if (ctrlKey == 'FormCheckbox' || (ctrlKey == "FormFormula" && ctrl.JsControl().BindType == ctrl.JsControl().BindControlType.Bool)) {
                        val.push(ctrl.JsControl().GetValue());
                    } else if (ctrlKey == 'FormUser') {
                        var uid = ctrl.JsControl().GetUnitIDs()[0];
                        val.push('"' + (uid == undefined ? "" : uid) + '"');
                    } else if (ctrlKey == 'FormMultiUser') {
                        //不支持多人
                        val.push('""');
                    } else if (ctrlKey == 'FormFormula') {
                        var ctrlVal = ctrl.JsControl().GetValue();
                        val.push('"' + ctrlVal + '"');
                    }
                    else {
                        //其他类型控件，取值后判断是否是输入，不是数字则转成字符串
                        for (var childFieldIndex = 0, childFieldLen = ctrl.length; childFieldIndex < childFieldLen; childFieldIndex++) {
                            var ctrlVal = $(ctrl[childFieldIndex]).JsControl().GetValue();
                            if ($.isNumeric(ctrlVal)) {
                                val.push(ctrlVal);
                            } else {
                                val.push('"' + ctrlVal + '"');
                ...
                if (ctrlField.indexOf('.') > -1) {
                    ...
                    for (var childFieldIndex = 0, childFieldLen = ctrls.length; childFieldIndex < childFieldLen; childFieldIndex++) {
                        if (ctrlKey == 'FormNumber') {
                            val.push($(ctrls[childFieldIndex]).JsControl().GetNum());
                        } else if (ctrlKey == 'FormTextBox' || ctrlKey == 'FormTextArea') {
                            var v = $(ctrls[childFieldIndex]).JsControl().GetValue();
                            val.push(v == "" ? "''" : ("'" + v + "'"));
                        } else if (ctrlKey == 'FormUser' || ctrlKey == 'FormMultiUser') {
                            var units = $(ctrls[childFieldIndex]).JsControl().GetUnitIDs();
                            ...
                            var area = $(ctrls[childFieldIndex]).JsControl().GetValue();
                            val.push(area == '' ? '""' : ("'" + area + "'"));
                        } 
                        else if (ctrlKey == 'FormCitySelect') {
                            var area = $(ctrls[childFieldIndex]).JsControl().GetValue();
                            val.push(area == '' ? '""' : ("'" + area + "'"));
                        }
                        else if (ctrlKey == 'FormAttachment' || ctrlKey == 'FormPhoto'||ctrlKey == 'FormFolder' ) {
                            var attach = $(ctrls[childFieldIndex]).JsControl().GetValue();
                            val.push(attach.AttachmentIds == '' ? "" : ('"' + attach.AttachmentIds + '"'));
                        } else if (ctrlKey == 'FormCheckbox') {
                            var c = $(ctrls[childFieldIndex]).JsControl().GetValue();
                            val.push(c);
                        } else if (ctrlKey == 'FormDropDownList') {
                            var dropDown = $(ctrls[childFieldIndex]).JsControl().GetValue();
                            val.push(dropDown == null ? '""' : ('"' + dropDown + '"'));
                        } else {
                            //其他类型控件，取值后判断是否是输入，不是数字则转成字符串
                            var ctrlVal = $(ctrls[childFieldIndex]).JsControl().GetValue();
                            if ($.isNumeric(ctrlVal)) {
                                val.push(ctrlVal);
                            } else {
                    ...
                    if (ctrlKey == 'FormNumber') {
                        ...
                        var v = ctrl.JsControl().GetValue();
                        val.push(v == '' ? "''" : ('"' + v + '"'));
                    } else if (ctrlKey == 'FormUser' || ctrlKey == 'FormMultiUser') {
                        var units = ctrl.JsControl().GetUnitIDs();
                        if (units != null && units != void 0 && units != "") {
                            for (var i = 0; i < units.length; i++) {
                                val.push('"' + units[i] + '"');
                            }
                        ...
                        var area = ctrl.JsControl().GetValue();
                        val.push(area == '' ? '""' : ("'" + area + "'"));
                    } else if (ctrlKey == 'FormAttachment' || ctrlKey == 'FormPhoto'||ctrlKey == 'FormFolder') {
                        var attach = ctrl.JsControl().GetValue();
                        val.push(attach.AttachmentIds == '' ? '' : ('"' + attach.AttachmentIds + '"'));
                    } else if (ctrlKey == 'FormCheckbox') {
                        var c = ctrl.JsControl().GetValue();
                        val.push(c);
                    } else if (ctrlKey == 'FormDropDownList') {
                        var dropDown = ctrl.JsControl().GetValue();
                        val.push(dropDown == null ? '""' : ('"' + dropDown + '"'));
                    } else if (ctrlKey == 'FormLabel') {
                        if (ctrlField == 'CreatedTime') {
                            val.push('"' + $.SmartForm.ResponseContext.ReturnData.CreatedTime.Value + '"');
        ...
        InitHideRule: function () {
            ...
            if (this.DataItem != null && this.DataItem.DisplayRule) {
                    ...
                    if (field.indexOf(".") > -1) {
                        if (this.DataField.indexOf(".") > -1) {
                            ...
                            for (var j = 0; j < tdCtrl.length; j++) {
                                var controlMgr = $(tdCtrl[j]).JsControl();
                                if (controlMgr) {
                                    controlMgr.UnbindChange(eventName);
                                    controlMgr.BindChange(eventName, function () {
                                        that.SetHideResult(rule, fields);
                        ...
                        if (ruleCtrl) {
                            ruleCtrl.BindChange('change.' + this.DataField, function () {
                                ...
                                for (var m = 0; m < thatCtrl.length; m++) {
                                    $(thatCtrl[m]).JsControl().SetVisible(!fun);
                                }
                                if (!fun) {
                                    if (canHideColumn) {
                ...
                if (this.DisplayRule.RuleDataField != void 0) {
                    ...
                    var ruleControl = $controls.JsControl();
                    if (ruleControl) {
