...
(function ($) {
    ...
    $.Controlkey = {
        ...
        Run: function (control, args) {
            ...
            //循环页面上的控件
            this.each(function () {
                if (!$(this).attr("data-" + $.Controlkey.SheetIDKey)) {
                    //edit by xc
                    p = $.myControls.GetDefaultOptions(control);
                    if (args.length > 0) {
                    ...
                    // 控件数量加1
                    $.Controlkey.ControlCount++;
                    // 添加控件属性
                    currentSheetIDKey = $.Controlkey.SheetIDKey + "-" + $.Controlkey.ControlCount.toString();
                    var datafield = $(this).attr("data-" + $.Controlkey.DataFieldKey.toLocaleLowerCase());
                    p[$.Controlkey.DataFieldKey] = $(this).attr("data-" + $.Controlkey.DataFieldKey.toLocaleLowerCase());
                    $(this).attr("data-" + $.Controlkey.SheetIDKey, currentSheetIDKey);
                    // new 控件
                    $.Controlkey.myControls[currentSheetIDKey] = new $.myControls[control](this, p, $.SmartForm.ResponseContext);
                }
                isOneControl++;
            });
            ...
            //如果是一个的话，返回当前控件管理器
            if (isOneControl == 1) {
                return $.Controlkey.myControls[$(this).attr("data-" + $.Controlkey.SheetIDKey)];
            }
        },
        ...
        SaveSheetData: function() {
            var SheetData = {};
            for (var control in this.myControls) {
                var controlManager = this.myControls[control];
                //过滤掉过滤条件中的控件
        ...
        GetElement: function (datafiled, bizObjectId) {
            ...
            if (datafiled.indexOf("#") == 0) {
                ...
                if (datafiled.indexOf('.') > -1) {
                    ...
                    this.GetElement(datafiled.split('.')[0]).JsControl();
                }
        ...
        GetControlValue: function (datafiled) {
            ...
            for (var i = 0; i < control.length; i++) {
                var manager = $(control[i]).JsControl();
                if (manager) {
        ...
        Validate: function (ActionControl) {
            ...
            for (var control in this.myControls) {
                ...
                if (ActionControl != null && ActionControl.Action == $.SmartForm.Action_Save && !(controlManager instanceof $.myControls.FormAttachment)) {
                    //保存的时候，需要校验附件是上传完整
                    continue;
                }
