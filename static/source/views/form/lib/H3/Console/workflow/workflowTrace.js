import {ActivitySettings} from './activity/setting';
...
export const TraceManager = {
    ...
    AtomicTrace: function (_TraceType, _Target, _Source) {
        ...
        var _svg= GlobalWorkflowProperties.SVG;
        switch (_TraceType) {
            case TraceManager.TraceType.Activity.Add:
                ...
                this.Redo = function () {
                    ...
                    if ($(_workflow.activities).filter(function () { return this.Id === _ID; }).length === 0) {
                        this.Activity.htmlObject = this.Html.clone()[0];
                        $(this.Activity.htmlObject).appendTo(_workflow.workspace);
                        _workflow.activities.push(this.Activity);
                ...
                this.Undo = function () {
                    ...
                    var _ID = this.Activity.Id;
                    var _index = 0;
                    $(_workflow.activities).each(function (i) {
                        if (this.Id === _ID) {
                            $(this.htmlObject).remove();
                            _index = i;
                        }
                    });
                    _workflow.activities.splice(_index, 1);
                };
            ...
            case TraceManager.TraceType.Line.Add:
                ...
                this.Redo = function () {
                    ...
                    if ($(_workflow.lines).filter(function () { return this.Id === _ID; }).length === 0) {
                        _workflow.lines.push(this.Line);
                        if (this.Line.Path) {
                            $(this.Line.Path).appendTo(_svg);
                        }
                        if (this.Line.Arrow) {
                            $(this.Line.Arrow).appendTo(_svg);
                        }
                        if (this.Line.Label) {
                            $(this.Line.Label).appendTo(_workflow.workspace);
                        }
                        this.Line.draw();
                    }
                };
                this.Undo = function () {
                    this.Line.Unselect();
                    var index = $.inArray(this.Line, _workflow.lines);
                    if (index > -1) {
                        $(_workflow.lines[index].Path).remove();
                        $(_workflow.lines[index].Arrow).remove();
                        $(_workflow.lines[index].Label).remove();
                        _workflow.lines.splice(index, 1);
                    }
                };
                break;
            case TraceManager.TraceType.Line.Remove:
                ...
                this.Undo = function () {
                    ...
                    this.Line.startActivity = $(_workflow.activities).filter(function () { return this.Id === _StartActivityID; })[0];
                    this.Line.endActivity = $(_workflow.activities).filter(function () { return this.Id === _EndActivityID; })[0];
                    //起止活动都存在
    ...
    Trace: function (_TraceType, _Target, _Source) {
        ...
        switch (_TraceType) {
            ...
            case TraceManager.TraceType.Activity.Add:
                if (!this.TraceText){
                    //this.TraceText = _WorkflowDesigner_GlobalString.WorkflowTrace_AddActivity;
                    this.TraceText = '新增节点';
                }
                break;
            case TraceManager.TraceType.Line.Add:
                if (!this.TraceText){
                    //this.TraceText = _WorkflowDesigner_GlobalString.WorkflowTrace_AddLine;
                    this.TraceText = '新增连接线';
                }
                break;
            case TraceManager.TraceType.Activity.TextChange:
                if (!this.TraceText){
                    //this.TraceText = _WorkflowDesigner_GlobalString.WorkflowTrace_ChangeText;
                    this.TraceText = '修改文本';
                }
                break;
            case TraceManager.TraceType.Activity.TextSizeChange:
                if (!this.TraceText){
                    //this.TraceText = _WorkflowDesigner_GlobalString.WorkflowTrace_ChangeTextSize;
                    this.TraceText = '修改文本字体大小';
                }
                break;
            case TraceManager.TraceType.Activity.TextColorChange:
                if (!this.TraceText){
                    //this.TraceText = _WorkflowDesigner_GlobalString.WorkflowTrace_ChangeTextColor;
                    this.TraceText = '修改文本颜色';
                }
                break;
                //线条
            case TraceManager.TraceType.Line.TextChange:
                if (!this.TraceText){
                    //this.TraceText = _WorkflowDesigner_GlobalString.WorkflowTrace_ChangeLineText;
                    this.TraceText = '修改连接线文本';
                }
                break;
            case TraceManager.TraceType.Line.TextSizeChange:
                if (!this.TraceText){
                    //this.TraceText = _WorkflowDesigner_GlobalString.WorkflowTrace_ChangeLineTextSize;
                    this.TraceText = '修改连接线文本字体大小';
                }
                break;
            case TraceManager.TraceType.Line.TextColorChange:
                if (!this.TraceText){
                    //this.TraceText = _WorkflowDesigner_GlobalString.WorkflowTrace_ChangeLineTextColor;
                    this.TraceText = '修改连接线文本颜色';
                }
            ...
            case TraceManager.TraceType.Line.PointChange:
                if (!this.TraceText){
                    // this.TraceText = _WorkflowDesigner_GlobalString.WorkflowTrace_AdjustLine;
                    this.TraceText= '调整连接线';
                }
                this.Line = _Target;
                this.AtomicTrace = new TraceManager.AtomicTrace(_TraceType, _Target, _Source);
                this.Redo = function () {
            ...
            case TraceManager.TraceType.Activity.Remove:
                if (!this.TraceText){
                    //this.TraceText = _WorkflowDesigner_GlobalString.WorkflowTrace_RemoveActivity;
                    this.TraceText= '移除节点';
                }
                this.Activity = _Target;
                var _ActivityID = this.Activity.Id;
                this.RelateLines = $(_workflow.lines).filter(function () { return this.startActivity.Id === _ActivityID || this.endActivity.Id === _ActivityID; });
                this.AtomicTraces = [];
            ...
            case TraceManager.TraceType.Line.Remove:
                if (!this.TraceText){
                   // this.TraceText = _WorkflowDesigner_GlobalString.WorkflowTrace_RemoveLine;
                   this.TraceText= '移除连接线';
                }
            ...
            case TraceManager.TraceType.Activity.Resize:
                if (!this.TraceText){
                    //this.TraceText = _WorkflowDesigner_GlobalString.WorkflowTrace_ChangeActivitySize;
                    this.TraceText= '修改活动节点大小';
                }
                break;
            case TraceManager.TraceType.Activity.Move:
                if (!this.TraceText){
                    //this.TraceText = _WorkflowDesigner_GlobalString.WorkflowTrace_MoveActivity;
                    this.TraceText= '移动活动节点';
                }
                break;
            case TraceManager.TraceType.Multi.SameHeight:
                if (!this.TraceText){
                    //this.TraceText = _WorkflowDesigner_GlobalString.Button_Heigth;
                    this.TraceText = '等高';
                }
                break;
            case TraceManager.TraceType.Multi.SameWidth:
                if (!this.TraceText){
                    //this.TraceText = _WorkflowDesigner_GlobalString.Button_Width;
                    this.TraceText = '等宽';
                }
                break;
            case TraceManager.TraceType.Multi.SameSize:
                if (!this.TraceText){
                    //this.TraceText = _WorkflowDesigner_GlobalString.Button_Size;
                    this.TraceText = '等大小';
                }
                break;
            case TraceManager.TraceType.Multi.VerticalDistance:
                if (!this.TraceText){
                    //this.TraceText = _WorkflowDesigner_GlobalString.Button_Vertical;
                    this.TraceText = '垂直等距';
                }
                break;
            case TraceManager.TraceType.Multi.HorizontalDistance:
                if (!this.TraceText){
                    //this.TraceText = _WorkflowDesigner_GlobalString.Button_Horizontal;
                    this.TraceText = '水平等距';
                }
                break;
            case TraceManager.TraceType.Multi.ActivitiesMove:
                if (!this.TraceText){
                    // this.TraceText = _WorkflowDesigner_GlobalString.WorkflowTrace_MoveActivityMulti;
                    this.TraceText = '批量移动节点';
                }
            ...
            case TraceManager.TraceType.Multi.ActivitiesTextSizeChange:
                if (!this.TraceText){
                    //this.TraceText = _WorkflowDesigner_GlobalString.WorkflowTrace_ActivitySize;
                    this.TraceText = '活动节点大小修改';
                }
            ...
            case TraceManager.TraceType.Multi.ActivitiesTextColorChange:
                if (!this.TraceText){
                    //this.TraceText = _WorkflowDesigner_GlobalString.WorkflowTrace_ActivityColor;
                    this.TraceText = '活动节点颜色变更';
                }
    ...
    Undo: function () {
        var _workflow=GlobalWorkflowProperties.Workflow;
        var _wp=GlobalWorkflowProperties.Wp;
        if (TraceManager.LastTraceIndex > -1) {
            ...
            if (!_wp.CurrentObject ||
                (_wp.CurrentObject.WorkflowElementType === WorkflowElementType.Activity && $.inArray(_wp.CurrentObject, _workflow.activities) === -1) ||
                (_wp.CurrentObject.WorkflowElementType === WorkflowElementType.Line && $.inArray(_wp.CurrentObject, _workflow.lines) === -1)) {
                    _wp.DisplayProperties(_workflow);
            }
        }
    },
    //重做
    ...
    AddTrace: function (_TraceType, _Target, _Source) {
        if (!TraceManager.UlPrevTrace || !TraceManager.UlNextTrace) {
            if (!$("#ulPrevTraces").get(0)) {
                $("<ul>").attr("id", "ulPrevTraces").appendTo("#btnUndo").hide();
            }
            if (!$("#ulNextTraces").get(0)) {
                $("<ul>").attr("id", "ulNextTraces").appendTo("#btnRedo").hide();
            }
        ...
        if (!TraceManager.ThumbnailReady) {
            ...
            if (ThumbnailFrame) {
                ...
                if (TraceManager.Thumbnail.workspace && TraceManager.Thumbnail.workspace.length === 1) {
                    ...
                    //滚动流程图容器更新导航器
                    $(_workflow.outerContainer).unbind('scroll.Thumbnail').bind('scroll.Thumbnail', function () {
    ...
    UpdateThumbnail: function () {
        ...
        if (TraceManager.ThumbnailReady) {
            //存在滚动条才显示缩略图
            if (_workflow.outerContainer.css('overflow-x') == 'scroll' || _workflow.outerContainer.css('overflow-y') === 'scroll') {
                if (!TraceManager.Thumbnail.Shown) {
                    TraceManager.Thumbnail.Panel.show();
                    return;
                }

                var _width = $(_workflow.workspace).width();
                var _height = $(_workflow.workspace).height();
    ...
    UpdateNav: function () {
        var _workflow=GlobalWorkflowProperties.Workflow;
        var p = TraceManager.Thumbnail.p;
        if (!p){
            return;
        }
            
        var _NavWidth = $(_workflow.outerContainer).width();
        var _NavHeight = $(_workflow.outerContainer).height();
        var _scale = 'scale(' + p + ')';
        TraceManager.Thumbnail.Nav
            ...
            .width(_NavWidth)
            .height(_NavHeight)
            .css('left', _NavWidth * (p - 1) / 2 + $(_workflow.outerContainer).scrollLeft() * p)
            .css('top', _NavHeight * (p - 1) / 2 + $(_workflow.outerContainer).scrollTop() * p);
    }
};
...
// WEBPACK FOOTER //
// ./src/lib/H3/Console/workflow/workflowTrace.js