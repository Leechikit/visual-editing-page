...
var AppTree = (function ($) {
    ...
    var config = {
        ...
        url:  baseUrl + '/Console/AppTree/OnAction', //异步取树url地址
    ...
    var ResetConfig = function () {
        ...
        config.url = baseUrl + '/Console/AppTree/OnAction';
        ...
        config.showWorkflowSystemField = false;
        config.curSheetCode = '';
        config.customCodes = [];
    ...
    var removeTree = function () {
        $(document).unbind('click.AppTree').bind('click.AppTree', function (event) {
            //判断范围
    ...
    var init = function (setting) {
        //重置参数
        ResetConfig();
        if(setting.showFolderFlag){
            config.url = baseUrl + '/fileManage/qryfileDirs';
        }
        //初始化参数赋值
        ...
        var zsetting = {
            async: {
                ...
                dataFilter: function (treeId, parentNode, data) {
                    ...
                    return data.ReturnData['ChildNodes'];
                }
            },
            callback: {
        ...
        if (config.root.appCode != null && config.root.appCode != "") {
            var znodes = [];
            if (config.curSheetCode != null && config.curSheetCode != "" && config.curSheetCode != config.root.appCode) {
                //添加并行根节点
                znodes.push({
            ...
            Parameter = { "PostData": JSON.stringify(Parameter) };
            $.ajax({
                type: "POST",
                url: "/Console/Reporting/OnAction",
                data: Parameter,
                dataType: "json",
                success: function (data) {
                    ...
                    var results = data.ReturnData['Results'];
                    if (results) {
...
// WEBPACK FOOTER //
// ./src/lib/H3/Console/SheetDesigner/AppTree.js