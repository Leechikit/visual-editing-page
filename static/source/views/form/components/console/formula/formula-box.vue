<template>
  <div class="formula-modal" v-if="showModal">
      <Row class="editor-container" :style="editorConHeight">
        <Col span="7" :style="editorConHeight1" class-name="apptree">
          <!--左侧数据模型-->
          <div v-for="treeData in appTreeData" class="root">
            <!-- 头部标题 -->
            <!-- 选择目标对象 -->
            <div v-if="showTargetSheet && treeData.IsTargetSchema" class="root-title" ref="targetSheet" @click="toggleChildren(treeData)">
            ...
            <!-- 对应子节点 -->
            <!-- 目标表单子节点 -->
            <div v-if="showTargetSheet && treeData.IsTargetSchema">
              <appTree v-show="treeData.expand" v-if="targetSheetTreeData[0] && targetSheetTreeData[0].children && targetSheetTreeData[0].children.length>0" :data="targetSheetTreeData" :parentData="treeData"
                :config="targetUrlParamsConfig" @onclick="chooseLeftTree" class="root-tree" :style="mainStyles">
              </appTree>
              ...
              <div class="sheet-container" v-show="showSearchSheet" :style="searchSheetStyles">
                ...
                <treeview :node="allTargetSheetRootNode" 
                        :hide-root-node="true"
                        :can-select="false" 
                        :level="0" 
                        :name-space="'formulabox-treeview'"
        ...
        <Col span="17" :style="editorConHeight1" class-name="formula-wrap">
...
import initEditorData from "./initEditorData.js";
import treeview from "@/views/form/lib/treeview";
import {
...
export default {
  ...
  data() {
    return {
      ...
      urlParamsConfig: {}, // 当前表单节点配置
      targetUrlParamsConfig: {}, // 目标表单节点配置
      ...
      targetAppId: "", // 目标表单appid
      allTargetSheetRootNode: {
        Name: "应用对象",
        Childrens: [],
        IsExpanded: true
      }
  ...
  created() {
    this.$root.eventHub.$on(
      "expand.formulabox-treeview",
      this.globalTreeViewNodeExpandHandler
    );
    this.$root.eventHub.$on(
      "click.formulabox-treeview",
      this.globalTreeViewNodeClickHandler
    );
    ...
    switch (this.FormulaType) {
      case FormulaTypeList.SCHEMACOMPUTATION: //计算规则
        ...
        try {
          rp = document
            .getElementById(this.FormulaField)
  ...
  destroyed(){
    this.$root.eventHub.$off('click.formulabox-treeview', this.globalTreeViewNodeClickHandler);
    this.$root.eventHub.$off('expand.formulabox-treeview', this.globalTreeViewNodeExpandHandler);
  },
  methods: {
    show() {
      this.showModal = true;
    ...
    async save() {
      ...
      let description = "";
      let ret = await validateFormula(this.SchemaCode,formula);
      if (!ret.Validate) {
        saveSuccessful = false;
        this.$refs.example.setAttribute("style", "display:none");
        this.$refs.description.setAttribute("style", "display:none");
    ...
    async initAppTree(setting) {
      ...
      HTTP.LoadAppTreeNodes({
        id: this.targetAppId || this.SchemaCode,
        CurSheetCode: this.SchemaCode
      }).then(json => {
        if (json.code == 0) {
          // 存在目标表单
          if (this.SchemaCode != this.targetAppId && this.targetAppId != "") {
            this.targetSheetTreeData = [{
              id: this.targetAppId,
              pId: this.targetAppId,
              name: this.targetSheetName,
              title: this.targetSheetName,
              Code: this.targetAppId,
              expand: true,
              NodeType: config.root.nodeType,
              iconSkin: config.root.icon,
              isParent: false,
              children: json.data.reWfTreeNodes,
              nocheck: true
            }];
          }
          this.appTreeData.push({
            ...
            iconSkin: config.root.icon,
            isParent: true,
            children: this.targetSheetTreeData,
            nocheck: true,
            IsTargetSchema: true
          });
          this.showTargetSheet = true;
          this.curSheetFileds = json.data.curReWfTreeNodes;
          // 增加当前表单字段
          if (this.curSheetFileds) {
            this.curSheetFileds.unshift({
              "id":"{currentAppRunId}",
              "pId":"root",
              "name":"当前表单",
              "title":"当前表单",
              "Code":"{currentAppRunId}",
              "NodeType":"Field",
              "ParentCode":null,
              "iconSkin":"icon-cgfk",
              "isParent":true,
    ...
    async toggleChildren(treeNode) {
      ...
      if (treeNode.children && treeNode.children.length == 0) {
        ...
        let json = await getAppTreeOnAction(params);
    ...
    async showFunctionsFun() {
      ...
      this.$nextTick(() => {
        ...
        if (this.$refs.showFunConBtn) {
          let style = $("style.formula-fun-container-style"),
            h =
              this.FormulaType.toUpperCase() == FormulaTypeList.DATASOURCE
                ? 30
                : 0,
            styleTop =
              this.$refs.showFunConBtn.offsetParent.offsetHeight + h + 30;
          if (style.length == 0) {
            let styleStr =
    ...
    async initEditor() {
      this.$nextTick(() => {
        let res = initEditorData;
        this.IntelligentFunctions =
          res.ReturnData.Parameter.IntelligentFunctions;
        ...
        loadParameter({
          ...
          .then(json => {
            if (json.code == 0 && json.datas) {
              const formulaText =
                this.BusinessType == 0
                  ? json.datas.submitText
                  : json.datas.rollbackText;
              const formulaCode =
                this.BusinessType == 0
                  ? json.datas.submitCode
                  : json.datas.rollbackCode;
              const matchCode = /\w+\({([^,}]+)}/.exec(formulaCode);
              const matchText = /\w+\({([^,}]+)}/.exec(formulaText);
              this.targetAppId = matchCode ? matchCode[1] : "";
              this.targetSheetName = matchText ? matchText[1] : "";
          ...
          .finally(() => {
            ...
            this.loadAppTreeNodes("root").then((nodes)=>{
              this.treeViewNodesLoaded(this.allTargetSheetRootNode,nodes);
            });
          });
      });
    },
    ...
    async _searchUnits() {
      const userRes = await SearchUsers(0, 8, this.unitSearchKey);
    ...
    getUrlParamsConfig(config) {
      ...
      let targetCf = {
        CurSheetCode: this.targetAppId
      }
      this.urlParamsConfig = cf;
      this.targetUrlParamsConfig = Object.assign({}, cf, targetCf);
    },
    ...
    async loadAppTreeNodes(id, code, nodeType) {
      ...
      return loadAppTreeNodes(params).then(json => {
        if (json.Successful) {
          return json.ReturnData.ChildNodes;
        }else{
          return []
        }
      });
    },
    treeViewNodesLoaded(checkedNode, children){
      let convertedNodes = this.convertToTreeViewNodes(
        children,
        checkedNode
      );
    ...
    globalTreeViewNodeExpandHandler(checkedNode, treeViewInstance) {
      this.globalTreeViewNodeClickHandler(checkedNode, treeViewInstance);
    },
    globalTreeViewNodeClickHandler(checkedNode, treeViewInstance) {
      const nodeType = checkedNode.UnitType;
      ...
      if(nodeType == "AppMenu" || nodeType == "SubSheet"){
        this.showSearchSheet = !this.showSearchSheet;
        this.targetAppId = checkedNode.Code;
        this.targetSheetName = checkedNode.Name;
        this.initAppTreeSetting();
        return;
      }
      
      if (
        checkedNode.IsExpanded ||
        (checkedNode.Childrens && checkedNode.Childrens.length)
      ) {
        checkedNode.IsExpanded = !checkedNode.IsExpanded;
        return;
      }
      this.loadAppTreeNodes(
        checkedNode.ObjectId,
        checkedNode.Code,
        checkedNode.UnitType
      ).then((children)=> {
        this.treeViewNodesLoaded(checkedNode,children);
        checkedNode.IsExpanded = !checkedNode.IsExpanded;
      });
    },
    hideAllTreeViewDropdown(){
      $(".sheet-container").find("ul").hide();
    }
  },
  mounted() {
    // delete key
