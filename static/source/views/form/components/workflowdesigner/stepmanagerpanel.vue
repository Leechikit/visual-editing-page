            ...
            <div class="select-set">
                <table>
                    <tr>
                        ...
                        <td>
                            <div class="value">
                                <div v-if="!$store.state.participant.StepManagerNode" style="background-color: transparent;padding: 0;color: #999;">请从下方选择部门或人员</div>
                                <div v-if="$store.state.participant.StepManagerNode">
                                    <template v-if="$store.state.participant.StepManagerNode.NodeType==0">
                                        <span class="icon-gongshi" v-if="$store.state.participant.StepManagerNode.UnitType===1"></span>
                                        <span class="icon-zhuzi" v-if="$store.state.participant.StepManagerNode.UnitType===2"></span>
                                        <span class="icon-renwu" v-if="$store.state.participant.StepManagerNode.UnitType===4"></span>
                                        <span>{{$store.state.participant.StepManagerNode.DisplayName ||
                                            $store.state.participant.StepManagerNode.displayName ||
                                            $store.state.participant.StepManagerNode.name}}</span>
                                        <span class="icon icon-close2" @click="removeCheck()"></span>
                                    </template>
                                    <template v-if="$store.state.participant.StepManagerNode.NodeType==1">
                                        <span>{{$store.state.participant.StepManagerNode.DisplayName ||
                                            $store.state.participant.StepManagerNode.displayName ||
                                            $store.state.participant.StepManagerNode.name}}</span>
                                        <span class="icon  icon-close2" @click="removeCheck()"></span>
...
<script>
    ...
    export default {
        name:'stepmanagerpanel',
        data(){
            return {
                show: false,
                departmentNameSpace:'STEPMANAGER-Department',
                userNameSpace:'STEPMANAGER-User',
                controlNameSpace:'STEPMANAGER-Control',
        ...
        methods:{
            ...
            ok(){
                ...
                //     return;
                // }
                // if(!this.$store.state.participant.StepManagerNode){
                ...
                let tmp=null;
                let node=this.$store.state.participant.StepManagerNode;
                ...
                // }else{
                    ...
                    tmp={
                        NodeType: NodeType.Function,
                        Formula: `RECURSIVEMANAGERSOF(${firstParam},${this.level||''},${
                            this.levelSearch})`,
                        DisplayName: `RECURSIVEMANAGERSOF(${node ? (node.DisplayName ||
                          node.displayName || node.name) : ''},${this.level||''},${this.levelSearch})`
                    }
                // }
                this.$emit("ok",tmp);
...
// WEBPACK FOOTER //
// src/components/workflowdesigner/stepmanagerpanel.vue