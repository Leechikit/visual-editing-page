...
export const saveFieldAcl = (schemaCode, draftFields, effectiveFields) => {
  ...
  const params = {
    appId: schemaCode,
    dataFieldState: JSON.stringify(myDraftData)
  };

  return fetch('/print/save', params, 'POST');
};
...
export const getAppTreeOnAction = (paramData) => {
  return fetch('/act/app/getFormInfo', paramData, 'POST');
};
...
//业务规则请求数据
export const getFormulaEventHandlerOnAction = (sheetCode) => {
  var params = {
    SheetCode: sheetCode
  };
  return fetch('/act/app/loadFormuBusinessRule', params, 'POST');
};
...
export const GetDepartmentUsers = (departmentId,pageSize) => {
  ...
  return fetch('/user/pageList',paramData);
};
/**
 * 获取子部门信息
 * @param {*} departmentId
...
export const GetChildDepartment = (departmentId) => {
  ...
  return fetch('/organ/childList', paramData);
};

/**
 * 获取参与者设置所需信息
 */
export const LoadParticipantInfos = (param) => {
  let paramData = {
    appId:param.appId,type:param.type,source:param.source
  };
  return fetch('/loadParticipantInfos', paramData);
};

export const validateFormula = (schemaCode, formula) => {
  // var params = {
  //   ActionName: "Validate",
  //   SchemaCode: schemaCode,
  //   Formula: formula,
  ...
  return fetch('/act/app/validateBusiness', params, 'POST');
};
 ...
 * @param {*} formulaField
 */
export const LoadNamesByUnitIds = (tmp) => {
  var params = {
    formula:tmp.formula,
    appId:tmp.appId,
    source:tmp.source,
...
// 业务规则回显
export const loadParameter = ({appId,num})=>{
  const params = {
    appId,
    num
  }
  return fetch('/act/app/loadParameter', params);
}
