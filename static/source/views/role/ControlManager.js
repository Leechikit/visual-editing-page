...
(function ($) {
    ...
    $.RoleControlManager = {
        ...
        Run: function (control, args) {
            ...
            //循环页面上的控件
            this.each(function () {
                if (!$(this).attr("data-" + $.RoleControlManager.SheetIDKey)) {
                    //edit by xc
                    p = $.RoleControls.GetDefaultOptions(control);
                    if (args.length > 0) {
                    ...
                    // 控件数量加1
                    $.RoleControlManager.ControlCount++;
                    // 添加控件属性
                    currentSheetIDKey = $.RoleControlManager.SheetIDKey + "-" + $.RoleControlManager.ControlCount.toString();
                    var datafield = $(this).attr("data-" + $.RoleControlManager.DataFieldKey.toLocaleLowerCase());
                    p[$.RoleControlManager.DataFieldKey] = $(this).attr("data-" + $.RoleControlManager.DataFieldKey.toLocaleLowerCase());
                    $(this).attr("data-" + $.RoleControlManager.SheetIDKey, currentSheetIDKey);
                    // new 控件
                    $.RoleControlManager.RoleControls[currentSheetIDKey] = new $.RoleControls[control](this, p, $.RoleSmartForm.ResponseContext);
                }
                isOneControl++;
            });
            ...
            //如果是一个的话，返回当前控件管理器
            if (isOneControl == 1) {
                return $.RoleControlManager.RoleControls[$(this).attr("data-" + $.RoleControlManager.SheetIDKey)];
            }
        },
        ...
        SaveSheetData: function() {
            var SheetData = {};
            for (var control in this.RoleControls) {
                var controlManager = this.RoleControls[control];
                //过滤掉过滤条件中的控件
        ...
        GetElement: function (datafiled, bizObjectId) {
            var element, $sheet;
            if ($.RoleSmartForm.ResponseContext.IsMobile) {
                $sheet = $.RoleSmartForm.ResponseContext.SheetView;
            }
            else {
                $sheet = $("#SheetContent");
            }
            if (datafiled.indexOf("#") == 0) {
                ...
                if (datafiled.indexOf('.') > -1) {
                    ...
                    this.GetElement(datafiled.split('.')[0]).RoleJControl();
                }
        ...
        GetControlValue: function (datafiled) {
            ...
            for (var i = 0; i < control.length; i++) {
                var manager = $(control[i]).RoleJControl();
                if (manager) {
        ...
        Validate: function (ActionControl) {
            ...
            for (var control in this.RoleControls) {
                ...
                if (ActionControl != null && ActionControl.Action == $.RoleSmartForm.Action_Save && !(controlManager instanceof $.RoleControls.FormAttachment)) {
                    //保存的时候，需要校验附件是上传完整
                    continue;
                }
...
// WEBPACK FOOTER //
// ./src/lib/H3/Form/ControlManager.js