...
(function ($) {
    ...
    $.fn.FormUser = function (opt) {
        return $.RoleControlManager.Run.call(this, "FormUser", arguments);
    };
    ...
    // 构造函数
    $.RoleControls.FormUser = function (element, ptions, sheetInfo) {
        ...
        this.FormUserHandler = $.RoleSmartForm.BaseApi + "/Form/OnAction"; //SheetUserAction
        this.Options = ptions;
        this.CpLock = false;
        $.RoleControls.FormUser.Base.constructor.call(this, element, ptions, sheetInfo);
        this.FromNum = 0;
        this.ToNum = 10;
    ...
    // 继承及控件实现
    $.RoleControls.FormUser.Inherit($.RoleControls.RoleBaseControl, {
        //控件渲染函数
        Render: function () {
        ...
        InitValue: function () {
            if (this.Value) {
                ...
                if ($.RoleSmartForm.ResponseContext.IsCreateMode) {
        ...
        HtmlRender: function () {
            ...
            this.$UserPanel.appendTo(this.CurrentBody);
        },
        ...
        RemoveChoice: function (UnitID, notApplyOnChange) {
            ...
            if (this.Units[UnitID]) {
                ...
                var typeStr = this.Units[UnitID].Type || this.Units[UnitID].UnitType;
                $(this.$UserPanel).find("i[data-UnitID='" + UnitID + "'][data-Type='" + typeStr + "']").removeClass("icon-Check").parent().data("Exist", false);
        ...
        LoadUnitsTree: function ($panel, tabType) {
            ...
            if (!$.isEmptyObject(that.UserData.OrgUnitItems)) {
                ...
                var params = { ActionName: "SheetUserAction", Command: actionCommand, UnitIDs: that.UnitSelectionRange };
                this.Ajax(
                    that.FormUserHandler,
                    "POST",
                    { PostData: JSON.stringify(params) },
        ...
        CreateUnitsItem: function (UnitItem, isDeps) {
            ...
            $a.append("<i class='" + icon + "'></i>").append(UnitItem.DisplayName).data("UnitItem", UnitItem);
            if (isDeps) {
                $li.addClass("SheetUser-LiItem");

                //如果是部门tab的话，炫耀可选
                var $stateIcon = $("<i class='glyphicon'></i>").attr({ "data-UnitID": UnitItem.UnitID, "data-Type": UnitItem.Type });
                $a.append($stateIcon);
                $a.click(function (e) {
                    ...
                    function (data) {
                        if (data.ReturnData.UnitItems.length > 0) {
                            for (var i = 0; i < data.ReturnData.UnitItems.length; i++) {
                                that.UserData.OrgUnitItems.push(data.ReturnData.UnitItems[i]);
                                units.push(data.ReturnData.UnitItems[i]);
                            }
                        }
                    }, false);
            }
        ...
        LoadUsersByParenID: function (ParentId) {
            ...
            if (!that.UserData.DepUserItems[ParentId]) {
                ...
                var params = {
                    ActionName: "SheetUserAction", Command: "LoadUsers", UnitID: ParentId, ShowUnActive: showUnActive
                };
                this.Ajax(
                    that.FormUserHandler,
                    "POST",
                ...
                for (var i = 0; i < that.UserData.DepUserItems[ParentId].length; i++) {
                    ...
                    var $a = $("<a>").append("<i class='glyphicon " + (UnitItem.Icon == "glyphicon-user" ? "icon-people" : UnitItem.Icon) + "'></i>").data("UnitItem", UnitItem);
...
// WEBPACK FOOTER //
// ./src/lib/H3/Form/Controls/FormUser.js