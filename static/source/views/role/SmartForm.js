...
jQuery.extend(
    $.RoleSmartForm,
    {
        ...
        Load: function (ResponseContext) {
            ...
            if (this.ResponseContext.IsMobile) {
                $.RoleControlManager.ClearControls();
        ...
        ValidateAction: function (ActionControl) {
            if ($.JForm._OnValidate(this, ActionControl)) {
                return $.RoleControlManager.Validate(ActionControl);
            }
            else {
                return false;
            }
            ...
            if (this.ResponseContext && !this.ResponseContext.IsMobile) {
                ...
                if (this.ResponseContext.ShowInModal) {
                    ...
                    for (var i in this.Actions) {
                        if (this.Actions[i].Action == "ViewQrCode" || this.Actions[i].Action == "Close") {
                            delete this.Actions[i];
                        }
                    }
                    $("ul[data-sheettoolbar-bottom='true']").FormToolBar(this.Actions);
                } else {
                ...
                if (datas) {
                    for (var i = 0; i < datas.length; i++) {
                        if (datas[i].toString().indexOf("{") == 0) {
                            ...
                            params.push($.RoleControlManager.GetControlValue(key));
                        }
                        else if (datas[i].toString().indexOf("#") == 0) {
                            var key = datas[i].replace("#");
                            params.push($.RoleControlManager.GetControlValue(datas[i]));
                        }
                        else {
                            params.push(datas[i]);
                        }
        ...
        Submit: function (actionControl, text, destActivity, postValue, groupValue) {
            ...
            that.PostForm(that.Action_Submit, { PostValue: JSON.stringify(SheetPostValue) }, function (data) {
                ...
                if (sheetQueryField && that.ResponseContext.FormDataType == that.SmartFormDataType.BizObject) {
                    ...
                    if (window.parent != window && window.parent.$.RoleSmartForm) {
                        window.parent.$.RoleSmartForm.WritebackSheetQuery(rowId, sheetQueryField, data.BizObjectId, data.InstanceName);
                    }
                }
                that.ResultHandler.apply(that, [actionControl, data]);
            });
        ...
        ShowHappyLoading: function () {
            ...
            var $nav = $(window.document.body).find("nav.sheet-navbar.navbar.navbar-fixed-top");
            var navHeight = $nav.height();
            var navMargin = parseInt($nav.css("margin-top")) + parseInt($nav.css("margin-bottom"));
            var $loading = $("<div class='loading-smartform'></div>").css({
                "height": (windowHeight - navHeight - navMargin) + "px",
                "width": windowWidth + "px",
                "position": "absolute",
                "z-index": 999,
                "top": navHeight + navMargin,
                "left": 0
            });
            //$loading绑定不允许鼠标滚动
            var firefox = navigator.userAgent.indexOf("FireFox") != -1;
            if (firefox) {
                $loading[0].addEventListener("DOMMouseScroll", function () { return false; }, false);
            } else {
                $loading[0].onmousewheel = function () { return false; };
            }
            var infos = ["系统正在玩命儿执行您的旨意", "阿里云正在计算那些无法计算的价值"];
            var random = Math.floor(Math.random() * 2);
            var $infoContainer = $("<div class='loadingInfo'><div>" + infos[random] + "</div><div>已持续<span class='wastedTime'>0</span>秒</div></div>");
            var $imageContainer = $("<div class='loadingImg'></div>");
            $loading.append($infoContainer).append($imageContainer);
            var cls = ["cutie", "bugme", "wastelife"];
            random = Math.floor(Math.random() * 3);
            $imageContainer.addClass(cls[random]);
            $sheet.parent().append($loading);
            //$sheet.hide();
            //启动计时器
            var wastedTime = 0;
            var that = this;
            that.loadingInterval && window.clearInterval(that.loadingInterval);
            setInterval(function () {
                $infoContainer.find("span").text(++wastedTime);
            }, 1000);
        },
        HideHappyLoading: function () {
            $("div.loading-smartform").remove();
            //var $sheet = $("#SheetContent");
            //$sheet.show();
        },
        // 关联查询新增，返写回界面
        WritebackSheetQuery: function (rowId, fieldName, ObjectId, displayName) {
            ...
            if ($element.length > 0) {
                var controlManager = $element.RoleJControl();
                if (controlManager) {
        ...
        ViewQrCode: function (actionControl) {
            ...
            //鼠标移开时隐藏二维码
            $QrCodeTip.off("mouseleave.tooltip").on("mouseleave.tooltip", function () {
                $QrCodeTip.hide();
            });
        ...
        GetPostValue: function (actionName, destActivity, postValue, groupValue) {
            ...
            SheetPostValue.Data = $.RoleControlManager.SaveSheetData();
            //区分移动端和pc端
            if (this.ResponseContext.IsMobile) { }
            else {
            ...
            SheetPostValue.Priority = $.RoleControlManager.Priority;
            SheetPostValue.HiddenFields = $.RoleControlManager.HiddenFields;
            return SheetPostValue;
        },
        ...
        ResultHandler: function (actionControl, data) {
            ...
            if (data.Successful) {
                ...
                if (data.Infos) {
                    ...
                    //不���足提交规则,修改标记，还可以继续提交
                    this.IsPosting = false;
                }
                //设置this.IsPosting=false后，再次提交会报"重复主键"的错误
                //this.IsPosting = false;
            }
        },
...
jQuery.extend({
    JForm: {
        ...
        _Init: function (form) {
            ...
            for (var key in $.RoleControlManager.RoleControls) {
                if ($.RoleControlManager.RoleControls[key].DataField != null && $.RoleControlManager.RoleControls[key].DataField.toString().indexOf(".") < 0) {
                    this[$.RoleControlManager.RoleControls[key].DataField] = $.RoleControlManager.RoleControls[key];
                }
            }
