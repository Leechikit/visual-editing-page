	...
	t.prototype = {
		Init: function() {
			...
			n(this.base[0]).off("click").on("click", function(i) {
				var u = n(i.target),
					r;
				return u.hasClass("chakanliebiao") ? (r = "/App/?id=" + t.schemaCode, window.open(r), i.stopPropagation(), !1) : u.hasClass("xinzengshuju") ? (r = "/Form/DefaultSheet?SchemaCode=" + t.schemaCode, n.ISideModal.Show(r), i.stopPropagation(), !1) : (r = "/Form/DefaultSheet?SchemaCode=" + t.schemaCode, n.ISideModal.Show(r), i.stopPropagation(), !1)
			});
...
$TextLabel = $("<label style='opacity:0;position:fixed;z-index:-1;'>").hide().appendTo("body");
 var i =function(n) {
	...
	n.fn.autocomplete = function(t) {
		var i, r;
		return arguments.length > 1 ? (i = t, t = arguments[1], t.url = i) : typeof t == "string" && (i = t, t = {
			url: i
		}), r = n.extend({}, n.fn.autocomplete.defaults, t), this.each(function() {
			var t = n(this);
			t.data("autocompleter", new n.Autocompleter(t, n.meta ? n.extend({}, r, t.data()) : r))
		})
	};
	...
	var i = function(t) {
			var i, r, u = typeof t;
			return u === "string" ? (i = t, r = {}) : n.isArray(t) ? (i = t[0], r = t.slice(1)) : u === "object" && (i = t.value, r = t.data), i = String(i), typeof r != "object" && (r = {}), {
				value: i,
				data: r
			}
		},
		t = function(n, t, i) {
			var r = parseInt(n, 10);
			return i = i || {}, (isNaN(r) || i.min && r < i.min) && (r = t), r
		},
		r = function(n, t) {
		...
		u = function(t, i) {
			...
			return n.each(i, function(n, t) {
				u.push(r(n, t))
			}), u.length && (t += t.indexOf("?") === -1 ? "?" : "&", t += u.join("&")), t
		},
		f = function(n, t, i) {
			return (n = String(n.value), t = String(t.value), i || (n = n.toLowerCase(), t = t.toLowerCase()), n > t) ? 1 : n < t ? -1 : 0
		},
		e = function(n, t, i) {
			for (var s = [], u, f, e, h, o = String(n).replace("\r\n", "\n").split(t), r = 0; r < o.length; r++) {
				for (e = o[r].split(i), f = [], u = 0; u < e.length; u++) f.push(decodeURIComponent(e[u]));
				h = f.shift();
				s.push({
					value: h,
					data: f
	...
	n.Autocompleter = function(i, r) {
		var u, f;
		if (!i || !(i instanceof n) || i.length !== 1 || i.get(0).tagName.toUpperCase() !== "INPUT") throw new Error("Invalid parameter for jquery.Autocompleter, jQuery object with one element with INPUT tag expected.");
		u = this;
		this.options = r;
		this.cacheData_ = {};
		this.cacheLength_ = 0;
		...
		this.options.minChars = t(this.options.minChars, n.fn.autocomplete.defaults.minChars, {
			min: 0
		});
		this.options.maxItemsToShow = t(this.options.maxItemsToShow, n.fn.autocomplete.defaults.maxItemsToShow, {
			min: 0
		});
		this.options.maxCacheLength = t(this.options.maxCacheLength, n.fn.autocomplete.defaults.maxCacheLength, {
			min: 1
		});
		this.options.delay = t(this.options.delay, n.fn.autocomplete.defaults.delay, {
			min: 0
		});
		this.options.preventDefaultReturn != 2 && (this.options.preventDefaultReturn = this.options.preventDefaultReturn ? 1 : 0);
		this.options.preventDefaultTab != 2 && (this.options.preventDefaultTab = this.options.preventDefaultTab ? 1 : 0);
		this.dom = {};
		this.dom.$elem = i;
		this.dom.$elem.attr("autocomplete", "off").addClass(this.options.inputClass);
		this.dom.$results = n("<div><\/div>").hide().addClass(this.options.resultsClass).css({
			position: "absolute"
		});
		n("body").append(this.dom.$results);
		i.keydown(function(n) {
			u.lastKeyPressed_ = n.keyCode;
			switch (u.lastKeyPressed_) {
			case u.options.delimiterKeyCode:
				u.options.useDelimiter && u.active_ && u.selectCurrent();
				break;
			case 35:
			case 36:
			case 16:
			...
			case 39:
				break;
			case 38:
				return n.preventDefault(), u.active_ ? u.focusPrev() : u.activate(), !1;
			case 40:
				return n.preventDefault(), u.active_ ? u.focusNext() : u.activate(), !1;
			case 9:
				if (u.active_ && (u.selectCurrent(), u.options.preventDefaultTab) || u.options.preventDefaultTab === 2) return n.preventDefault(), !1;
				break;
			case 13:
				if (u.active_ && (u.selectCurrent(), u.options.preventDefaultReturn) || u.options.preventDefaultReturn === 2) return n.preventDefault(), !1;
				break;
			case 27:
				if (u.active_) return n.preventDefault(), u.deactivate(!0), !1;
				break;
			default:
				u.activate()
			}
		...
		i.blur(function() {
			u.finishOnBlur_ && (u.finishTimeout_ = setTimeout(f, 200))
		});
		i.parents("form").on("submit", f)
	};
	n.Autocompleter.prototype.position = function() {
		var t = this.dom.$elem.offset(),
			r = this.dom.$results.outerHeight(),
			e = n(window).outerHeight(),
			u = t.top + this.dom.$elem.outerHeight(),
			o = u + r,
			f = {
				top: u,
				left: t.left
			...
			i;
		o > e && (i = t.top - r, i >= 0 && (f.top = i));
		this.dom.$results.css(f)
	};
	n.Autocompleter.prototype.cacheRead = function(n) {
		var i, t, r, f, u;
		if (this.options.useCache) for (n = String(n), i = n.length, t = this.options.matchSubset ? 1 : i; t <= i;) {
			for (f = this.options.matchInside ? i - t : 0, u = 0; u <= f;) {
				if (r = n.substr(0, t), this.cacheData_[r] !== undefined) return this.cacheData_[r];
				u++
			}
			t++
		}
	...
	n.Autocompleter.prototype.cacheWrite = function(n, t) {
		return this.options.useCache ? (this.cacheLength_ >= this.options.maxCacheLength && this.cacheFlush(), n = String(n), this.cacheData_[n] !== undefined && this.cacheLength_++, this.cacheData_[n] = t, this.cacheData_[n]) : !1
	};
	n.Autocompleter.prototype.cacheFlush = function() {
		this.cacheData_ = {};
		this.cacheLength_ = 0
	};
	n.Autocompleter.prototype.callHook = function(t, i) {
		var r = this.options[t];
		return r && n.isFunction(r) ? r(i, this) : !1
	};
	n.Autocompleter.prototype.activate = function() {
	...
	n.Autocompleter.prototype.activateNow = function() {
		var n = this.beforeUseConverter(this.dom.$elem.val());
		n !== this.lastProcessedValue_ && n !== this.lastSelectedValue_ && this.fetchData(n)
	};
	n.Autocompleter.prototype.fetchData = function(n) {
		var t = this,
			i = function(n, i) {
				t.options.processData && (n = t.options.processData(n));
				t.showResults(t.filterResults(n, i), i)
			};
		this.lastProcessedValue_ = n;
		n.length < this.options.minChars ? i([], n) : this.options.data ? i(this.options.data, n) : this.fetchRemoteData(n, function(t) {
			i(t, n)
		})
	};
	n.Autocompleter.prototype.fetchRemoteData = function(t, i) {
		var u = this.cacheRead(t);
		if (u) i(u);
		else {
			var r = this,
				e = r.options.remoteDataType === "json" ? "json" : "text",
				f = function(n) {
					var u = !1;
					n !== !1 && (u = r.parseRemoteData(n), r.cacheWrite(t, u));
					r.dom.$elem.removeClass(r.options.loadingClass);
					i(u)
				};
			...
			n.ajax({
				...
				error: function(t, i, u) {
					if (n.isFunction(r.options.onError)) r.options.onError(t, i, u);
					else f(!1)
				},
				dataType: e
			})
	...
	n.Autocompleter.prototype.setExtraParam = function(t, i) {
		var r = n.trim(String(t));
		r && (this.options.extraParams || (this.options.extraParams = {}), this.options.extraParams[r] !== i && (this.options.extraParams[r] = i, this.cacheFlush()))
	};
	n.Autocompleter.prototype.makeUrl = function(t) {
		var f = this,
			i = this.options.url,
			r = n.extend({}, this.options.extraParams);
		return this.options.queryParamName === !1 ? i += encodeURIComponent(t) : r[this.options.queryParamName] = t, u(i, r)
	};
	n.Autocompleter.prototype.parseRemoteData = function(t) {
		var r, i = t;
		if (this.options.remoteDataType === "json") {
			...
			case "string":
				...
				break;
			default:
				throw new Error("Unexpected remote data type: " + r);
			}
			return i
		}
		return e(i, this.options.lineSeparator, this.options.cellSeparator)
	};
	n.Autocompleter.prototype.defaultFilter = function(n, t) {
		var i, r, u;
		return n.value ? this.options.filterResults ? (i = this.matchStringConverter(t), r = this.matchStringConverter(n.value), this.options.matchCase || (i = i.toLowerCase(), r = r.toLowerCase()), u = r.indexOf(i), this.options.matchInside ? u > -1 : u === 0) : !0 : !1
	};
	n.Autocompleter.prototype.filterResult = function(t, i) {
		return this.options.filter === !1 ? !0 : n.isFunction(this.options.filter) ? this.options.filter(t, i) : this.defaultFilter(t, i)
	};
	n.Autocompleter.prototype.filterResults = function(n, t) {
		for (var r = [], f, u = 0; u < n.length; u++) f = i(n[u]), this.filterResult(f, t) && r.push(f);
		return this.options.sortResults && (r = this.sortResults(r, t)), this.options.maxItemsToShow > 0 && this.options.maxItemsToShow < r.length && (r.length = this.options.maxItemsToShow), r
	};
	n.Autocompleter.prototype.sortResults = function(t, i) {
		var u = this,
			r = this.options.sortFunction;
		return n.isFunction(r) || (r = function(n, t) {
			return f(n, t, u.options.matchCase)
		}), t.sort(function(n, t) {
			return r(n, t, i, u.options)
		}), t
	};
	n.Autocompleter.prototype.matchStringConverter = function(t, i, r) {
		var u = this.options.matchStringConverter;
		return n.isFunction(u) && (t = u(t, i, r)), t
	};
	n.Autocompleter.prototype.beforeUseConverter = function(t) {
		t = this.getValue(t);
		var i = this.options.beforeUseConverter;
		return n.isFunction(i) && (t = i(t)), t
	};
	n.Autocompleter.prototype.enableFinishOnBlur = function() {
		this.finishOnBlur_ = !0
	};
	n.Autocompleter.prototype.disableFinishOnBlur = function() {
		this.finishOnBlur_ = !1
	};
	n.Autocompleter.prototype.createItemFromResult = function(t) {
		var r = this,
			i = n("<li/>");
		return i.text(this.showResult(t.value, t.data)), i.data({
			value: t.value,
			data: t.data
		}).click(function() {
			r.selectItem(i)
		}).mousedown(r.disableFinishOnBlur).mouseup(r.enableFinishOnBlur), i
	};
	n.Autocompleter.prototype.getItems = function() {
		return n(">ul>li", this.dom.$results)
	};
	n.Autocompleter.prototype.showResults = function(t, i) {
		var f = t.length,
			l = this,
			s = n("<ul><\/ul>"),
			r, e, u, h, o = !1,
			c = !1;
		if (f) {
			for (r = 0; r < f; r++) e = t[r], u = this.createItemFromResult(e), s.append(u), o === !1 && (o = String(e.value), c = u, u.addClass(this.options.firstItemClass)), r === f - 1 && u.addClass(this.options.lastItemClass);
			this.dom.$results.html(s).show();
			this.position();
			this.options.autoWidth && (h = this.dom.$elem.outerWidth() - this.dom.$results.outerWidth() + this.dom.$results.width(), this.dom.$results.css(this.options.autoWidth, h));
			this.getItems().hover(function() {
				l.focusItem(this)
			}, function() {});
			(this.autoFill(o, i) || this.options.selectFirst || this.options.selectOnly && f === 1) && this.focusItem(c);
			this.active_ = !0
		} else this.hideResults(), this.active_ = !1
	};
	n.Autocompleter.prototype.showResult = function(t, i) {
		return n.isFunction(this.options.showResult) ? this.options.showResult(t, i) : t
	};
	n.Autocompleter.prototype.autoFill = function(n, t) {
		var f, e, o, u, i, r, s, h;
		return this.options.autoFill && this.lastKeyPressed_ !== 8 && (f = String(n).toLowerCase(), e = String(t).toLowerCase(), o = n.length, u = t.length, f.substr(0, u) === e) ? (i = this.getDelimiterOffsets(), r = i.start ? " " : "", this.setValue(r + n), s = u + i.start + r.length, h = o + i.start + r.length, this.selectRange(s, h), !0) : !1
	};
	n.Autocompleter.prototype.focusNext = function() {
		this.focusMove(1)
	};
	n.Autocompleter.prototype.focusPrev = function() {
		this.focusMove(-1)
	};
	n.Autocompleter.prototype.focusMove = function(i) {
		var u = this.getItems(),
			r;
		if (i = t(i, 0), i) for (r = 0; r < u.length; r++) if (n(u[r]).hasClass(this.selectClass_)) {
			this.focusItem(r + i);
			return
		}
		this.focusItem(0)
	};
	n.Autocompleter.prototype.focusItem = function(t) {
		var r, i = this.getItems();
		i.length && (i.removeClass(this.selectClass_).removeClass(this.options.selectClass), typeof t == "number" ? (t < 0 ? t = 0 : t >= i.length && (t = i.length - 1), r = n(i[t])) : r = n(t), r && r.addClass(this.selectClass_).addClass(this.options.selectClass))
	};
	n.Autocompleter.prototype.selectCurrent = function() {
		var t = n("li." + this.selectClass_, this.dom.$results);
		t.length === 1 ? this.selectItem(t) : this.deactivate(!1)
	};
	n.Autocompleter.prototype.selectItem = function(n) {
		var f = n.data("value"),
			e = n.data("data"),
			t = this.displayValue(f, e),
			o = this.beforeUseConverter(t);
		this.lastProcessedValue_ = o;
		this.lastSelectedValue_ = o;
		var i = this.getDelimiterOffsets(),
			r = this.options.delimiterChar,
			u = this.dom.$elem,
			s = 0;
		this.options.useDelimiter && (u.val().substring(i.start - 1, i.start) == r && r != " " && (t = " " + t), u.val().substring(i.end, i.end + 1) != r && this.lastKeyPressed_ != this.options.delimiterKeyCode ? t = t + r : s = 1);
		this.setValue(t);
		this.setCaret(i.start + t.length + s);
	...
	n.Autocompleter.prototype.displayValue = function(t, i) {
		return n.isFunction(this.options.displayValue) ? this.options.displayValue(t, i) : t
	};
	n.Autocompleter.prototype.hideResults = function() {
		this.dom.$results.hide()
	};
	n.Autocompleter.prototype.deactivate = function(n) {
		this.finishTimeout_ && clearTimeout(this.finishTimeout_);
		this.keyTimeout_ && clearTimeout(this.keyTimeout_);
		n && (this.lastProcessedValue_ !== this.lastSelectedValue_ && (this.options.mustMatch && this.setValue(""), this.callHook("onNoMatch")), this.active_ && this.callHook("onFinish"), this.lastKeyPressed_ = null, this.lastProcessedValue_ = null, this.lastSelectedValue_ = null, this.active_ = !1);
		this.hideResults()
	};
	n.Autocompleter.prototype.selectRange = function(n, t) {
		var i = this.dom.$elem.get(0),
			r;
		i.setSelectionRange ? (i.focus(), i.setSelectionRange(n, t)) : i.createTextRange && (r = i.createTextRange(), r.collapse(!0), r.moveEnd("character", t), r.moveStart("character", n), r.select())
	};
	n.Autocompleter.prototype.setCaret = function(n) {
		this.selectRange(n, n)
	};
	n.Autocompleter.prototype.getCaret = function() {
		var r = this.dom.$elem,
			o = r[0],
			t, u, n, i, f, e;
		return o.createTextRange ? (u = document.selection, o.tagName.toLowerCase() != "textarea" ? (t = r.val(), n = u.createRange().duplicate(), n.moveEnd("character", t.length), i = n.text === "" ? t.length : t.lastIndexOf(n.text), n = u.createRange().duplicate(), n.moveStart("character", -t.length), f = n.text.length) : (n = u.createRange(), e = n.duplicate(), e.moveToElementText(o), e.setEndPoint("EndToEnd", n), i = e.text.length - n.text.length, f = i + n.text.length)) : (i = r[0].selectionStart, f = r[0].selectionEnd), {
			start: i,
			end: f
		}
	};
	n.Autocompleter.prototype.setValue = function(n) {
		if (this.options.useDelimiter) {
			var t = this.dom.$elem.val(),
				i = this.getDelimiterOffsets(),
				r = t.substring(0, i.start),
				u = t.substring(i.end);
			n = r + n + u
		}
		this.dom.$elem.val(n)
	};
	n.Autocompleter.prototype.getValue = function(n) {
		if (this.options.useDelimiter) {
			var t = this.getDelimiterOffsets();
			return n.substring(t.start, t.end).trim()
		}
		return n
	};
	n.Autocompleter.prototype.getDelimiterOffsets = function() {
		var t = this.dom.$elem.val();
		if (this.options.useDelimiter) {
			var r = t.substring(0, this.getCaret().start),
				i = r.lastIndexOf(this.options.delimiterChar) + 1,
				u = t.substring(this.getCaret().start),
				n = u.indexOf(this.options.delimiterChar);
			n == -1 && (n = t.length);
			n += this.getCaret().start
		} else i = 0, n = t.length;
		return {
			start: i,
			end: n
...
$(function() {
	$("a.head-search-btn").unbind("click").bind("click", function() {
		var n = $("#txtSearch").val();
		window.location.href = "/GlobalSearch/Index?Keyword=" + encodeURIComponent(n)
	});
	$("span.search").unbind("click").bind("click", function() {
		var n = $("#txtSearch_top").val();
		window.location.href = "/GlobalSearch/Index?Keyword=" + encodeURIComponent(n)
	});
	$("#txtSearch_top").autocomplete({
		url: "/GlobalSearch/AutoComplete",
		minChars: 1,
		onItemSelect: function(n) {
			window.location.href = "/GlobalSearch/Index?Keyword=" + encodeURIComponent(n.value)
		}
	});
	$("#txtSearch_top").unbind("keyup").bind("keyup", function(n) {
		if (n.which == 13) {
			var t = $("#txtSearch_top").val();
			return window.location.href = "/GlobalSearch/Index?Keyword=" + encodeURIComponent(t), !1
		}
	});
	var n = $(".results-list");
	$(".box-recent").each(function() {
		var t = $(this),
			i = t.attr("data-appcode"),
			r;
		i == "" || i == "Sys_Workflow" ? (r = t.attr("data-code"), t.unbind("click").bind("click", function() {
			var n = "/Form/DefaultSheet/?SchemaCode=" + r;
			Open(n)
		})) : t.NewOrCheckList({
			schemaCode: $(this).attr("data-code"),
			margin: 7,
			scrollDom: n
		})
	})
