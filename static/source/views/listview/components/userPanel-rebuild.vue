<template>
  <div class="userpanel-box">
    <div class="form-control form-user-add" @click="showUserPanel">
      ...
      <span
        class="SheetUser-Item label label-info icon-close-middle"
    ...
    <div
      class="userpanel"
      v-show="isShowUserPanel"
      :style="{ left: `${userpanelLeft}px` }"
    >
      <!-- <div class="searchdiv">
      ...
      <div class="SheetUser-SelectorPanel" v-show="isShowUserPanel">
        ...
        <div
          class="SheetUser_DataPanel row SheetUser_tab_Users"
          v-show="activeIndex === 0"
        >
          <div
            class="SheetUser_TreePanel col-sm-7 col-xs-7"
            style="height: 100%;"
          >
            ...
            <ul class="nav">
              <li v-for="item in userData" class="SheetUser-LiItem">
                <a
                  ...
                  @click="selectUser(item.DisplayName, item.id)"
                ></a>
              </li>
            </ul>
          </div>
        ...
        <div
          class="SheetUser_DataPanel row SheetUser_tab_Deps"
          v-show="activeIndex === 1"
        >
          <div
            class="SheetUser_TreePanel col-sm-12 col-xs-12"
            style="height: 100%;"
          >
            <Tree :data="depTreeData" :load-data="loadDepTreeData"></Tree>
          </div>
  ...
  methods: {
    ...
    showUserPanel($event) {
      ...
      if (this.isShowUserPanel) {
        ...
        const inputLeft = $($event.target).offset().left
        const panelWidth = 500
        if (winWidth - inputLeft > panelWidth) {
          this.userpanelLeft = 0
        } else {
          this.userpanelLeft = winWidth - inputLeft - panelWidth
        }
        this.$nextTick(() => {
          setTimeout(() => {
