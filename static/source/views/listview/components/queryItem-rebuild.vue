  ...
  <div class="query-item">
    ...
    <span class="query-right">
      <Input
        ...
        @on-change="change($event.target.value)"
      ></Input>
      <Input
      ...
      >
        <Option
          v-for="(item, $index) in optionalArrs"
          :value="item"
          :key="$index"
          >{{ item }}</Option
        >
      ...
      <user-panel
        v-if="queryType === 'user' || queryType === 'multiUser'"
        ref="userPanel"
        :readonly="readonly"
        type="user"
        @on-change="change"
      ></user-panel>
      <user-panel
        v-if="queryType === 'dep' || queryType === 'multiDep'"
        ref="depPanel"
        :readonly="readonly"
        type="dep"
        @on-change="change"
...
<script>
import userPanel from './userPanel-rebuild.vue'
export default {
  components: {
    userPanel
  },
  ...
  methods: {
    change(value) {
      ...
      if (
        ...
        (this.queryType === 'select' || this.queryType === 'checkbox')
      ) {
        value = value.join(';')
      }
    ...
    reset() {
      this.currentValue = ''
      this.checkbox = []
      if (this.queryType === 'user' || this.queryType === 'multiUser') {
        this.$refs.userPanel.reset()
      }
      if (this.queryType === 'dep' || this.queryType === 'multiDep') {
        this.$refs.depPanel.reset()
      }
      this.$emit('change', '')
    }
  }
