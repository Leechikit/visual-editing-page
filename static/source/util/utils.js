...
/* 防抖函数 */
export const debounce = function (fun = () => {}, elaspe = 500, context) {
...
/* 节流函数 */
export const throttle = function (fun = () => {}, elaspe = 200, context) {
  let lastTime = 0;
  return function (...args) {
    let nowTime = +new Date();
    if (nowTime - lastTime > elaspe) {
      fun.apply(context, args);
      lastTime = nowTime;
    }
  };
...
export const changeToDingDingUrl = function (url){
   if(url==void 0){
     return ""
   }
   else{
    ...
    url = ip+url
    return "dingtalk://dingtalkclient/action/openapp?container_type=work_platform&redirect_type=jump&redirect_url="+encodeURIComponent("http://"+url)
   }
}
